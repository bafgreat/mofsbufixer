...
def test_inchikey(organic_sbu, metal_sbu):
    ...
    '''
    x_indices_org, organic_h_sbu = compile_sbu.sub_dummy_with_hydrogens(
        organic_sbu)
    x_indices_met, metal_h_sbu = compile_sbu.sub_dummy_with_hydrogens(
        metal_sbu)
    organic_inchi_key = compile_sbu.ase_to_inchi(organic_h_sbu)
    formatted_name, iupac_nanme = compile_sbu.inchikey_to_name(
        organic_inchi_key)
